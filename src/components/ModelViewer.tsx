...
import { Suspense, useRef, useLayoutEffect, useEffect, useMemo, Component, ReactNode } from 'react';
  ...
  return (
    <Html center>
      {placeholderSrc ? (
        ...
        <div className="text-gold-400 font-semibold">{Math.round(progress)}%</div>
      )}
    </Html>
  );
};
...
const DesktopControls = ({ pivot, min, max, zoomEnabled, rotateEnabled, autoRotate, autoRotateSpeed }: DesktopControlsProps) => {
...
const ModelInner = ({
  ...
  useFrame((_, dt) => {
    ...
    cHov.current.y += (tHov.current.y - tHov.current.y) * HOVER_EASE;
  ...
  return (
    ...
    >
      {showScreenshotButton && (
        <button
          onClick={capture}
          className="absolute top-4 right-4 z-10 px-4 py-2 border border-gold-400/50 rounded-xl bg-black/50 text-gold-400 hover:bg-gold-400/20 transition-colors"
        >
          Capturar
        </button>
      )}
      ...
      <ModelErrorBoundary onError={onModelError}>
        ...
        >
          ...
          <directionalLight position={[5, 5, 5]} intensity={keyLightIntensity} castShadow color="#ffd700" />
          <directionalLight position={[-5, 2, 5]} intensity={fillLightIntensity} color="#ffffff" />
          <directionalLight position={[0, 4, -5]} intensity={rimLightIntensity} color="#ffd700" />

          <ContactShadows ref={contactRef} position={[0, -0.5, 0]} opacity={0.35} scale={10} blur={2} color="#ffd700" />
          ...
          {/* Controls for both desktop and touch */}
          <DesktopControls 
            pivot={pivot} 
            min={minZoomDistance} 
            max={maxZoomDistance} 
            zoomEnabled={enableManualZoom}
            rotateEnabled={enableManualRotation}
            autoRotate={autoRotate}
            autoRotateSpeed={autoRotateSpeed}
          />
        </Canvas>
