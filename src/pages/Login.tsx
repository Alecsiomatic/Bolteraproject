...
const Login = () => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      toast({ title: "Ingreso exitoso", description: "Bienvenido de vuelta" });
      navigate(fromRoute ?? getRedirectPath(loggedUser?.role), { replace: true });
    } catch (error) {
  ...
  const handleQuickLogin = async (userEmail: string, userPassword: string, userName: string) => {
    setSubmitting(true);
    try {
      const loggedUser = await login(userEmail, userPassword);
      toast({ title: "Ingreso exitoso", description: `Bienvenido ${userName}` });
      navigate(fromRoute ?? getRedirectPath(loggedUser?.role), { replace: true });
    } catch (error) {
      const message = error instanceof Error ? error.message : "No se pudo iniciar sesión";
      toast({ variant: "destructive", title: "Error", description: message });
  ...
  const isDisabled = submitting || loading || !email || !password;
        ...
        >
          ...
          <div className="relative z-10 space-y-6 sm:space-y-8">
            ...
            <div className="text-center space-y-3 sm:space-y-4">
              ...
              />
              <div>
                <p className="text-xs sm:text-sm uppercase tracking-[0.3em] sm:tracking-[0.4em] text-gold-400 mb-1">Bienvenido</p>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-xs uppercase">
                <span className="px-3 text-white/40 bg-transparent backdrop-blur-sm">Acceso rápido</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              <Button
                type="button"
                variant="outline"
                className="h-12 rounded-2xl border-gold-500/20 bg-black/30 text-gold-400 hover:bg-gold-500/10 hover:border-gold-500/40 backdrop-blur-sm"
                onClick={() => handleQuickLogin("admin@boletera.com", "Admin123!", "Admin")}
              ...
              <Button
                type="button"
                variant="outline"
                className="h-12 rounded-2xl border-amber-500/20 bg-black/30 text-amber-400 hover:bg-amber-500/10 hover:border-amber-500/40 backdrop-blur-sm"
                onClick={() => handleQuickLogin("user@boletera.com", "User123!", "Usuario")}
                disabled={submitting || loading}
              >
