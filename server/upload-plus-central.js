...
const SECTION_PREFIX = 'plus-central';
...
async function main() {
  ...
  console.log('=== ACTUALIZAR PLUS CENTRAL ===\n');
  console.log('Modo:', UPLOAD ? '🚀 UPLOAD REAL' : '👁️ PREVIEW (usa --upload para subir)');
  
  // Cargar asientos corregidos
  const newSeats = JSON.parse(fs.readFileSync('server/plus-central-fixed.json', 'utf8'));
  ...
  const targetSection = sections.find(s => s.name === 'PLUS CENTRAL');
  
  console.log('Section ID:', targetSection?.id);
  ...
  console.log('Asientos PLUS CENTRAL actuales:', targetCurrentIds.size);
  ...
  console.log('Asientos PLUS CENTRAL nuevos:', apiNewSeats.length);
  ...
  const payload = {
    layoutId: LAYOUT_ID,
    layoutJson: {
      canvas: { ...canvas, objects: finalCanvasObjects },
      zones: zones,
      sections: sections
    },
    seats: allSeats
  ...
  if (saveResponse.statusCode === 200 || saveResponse.statusCode === 201) {
    ...
    console.log('PLUS CENTRAL:', verifyTarget.length);
  } else {
